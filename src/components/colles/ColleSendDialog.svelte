  ...
  $effect(() => {
    ...
    void (async () => {
      try {
        ...
        missing = miss;
        phase = "ready";
        // Rendu de FOND : démarre IMMÉDIATEMENT (décision utilisateur round 20
        // — le cache est produit au lancement du dialogue, pas au clic).
        // Le dialogue reste utilisable ; `startRender` remplit `previewImages`
        // au fil de l'eau. Les captures headless restent séquentielles
        // (onglet unique partagé + backend sérialisé).
        startRender();
      } catch (err) {
        if (cancelled) return;
