<script lang="ts">
  ...
  let Cmp = $state<typeof import("./SynctexPdfViewer.svelte").default | null>(null);

  $effect(() => {
    let cancelled = false;
    import("./SynctexPdfViewer.svelte")
      .then((mod) => {
        if (cancelled) return;
        Cmp = mod.default;
      })
      .catch((err) => {
        if (cancelled) return;
        console.error("azprose: failed to load Synctex PDF viewer", err);
      });
