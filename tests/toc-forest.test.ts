...
/**
 * Tests du harnais de sécurité TOC : arbre des titres du fichier de
 * RÉFÉRENCE (toujours le fichier affiché — la remontée « index.md lié »
 * (Home) a été retirée au chantier fenêtre NAV, phase 3 : elle était déjà
 * désactivée en pratique depuis la Phase G, seul appelant `TocPanel` passant
 * toujours `linkedIndex: false`), étendu des branches transcluses des
 * wikilinks block-level CONFORMES (même convention que l'export PDF —
 * findBlockWikilinks).
 */
...
describe("buildTocForest — fichier affiché = TOUJOURS la référence", () => {
  test("displayPath = referencePath, contenu depuis referenceSource (buffer live)", async () => {
  ...
  test("sans referenceSource → lecture disque du fichier de référence", async () => {
  ...
  test("référence illisible → root null, jamais de repli sur un autre fichier", async () => {
    const forest = await build("/vault/notes/absent.md", { readText: fakeFs({}) });
    expect(forest.root).toBeNull();
    expect(forest.displayPath).toBe("/vault/notes/absent.md");
  });
});
...
describe("buildTocForest — branches transcluses (wikilinks conformes)", () => {
  ...
  test("wikilink inline (non conforme) → aucune branche", async () => {
    ...
    const forest = await build("/vault/index.md", { readText: fs });
  ...
  test("maxDepth: 0 → aucune branche transcluse", async () => {
    const forest = await build("/vault/index.md", { maxDepth: 0 });
    expect(collectBranches(forest.root!)).toHaveLength(0);
  });

  test("maxDepth: 1 → branche de niveau 1 seule (pas de niveau 2)", async () => {
    const forest = await build("/vault/index.md", { maxDepth: 1 });
  ...
  test("résolution relative au fichier hôte", async () => {
    ...
    const forest = await build("/vault/notes/index.md", { readText: fs });
  ...
  test("repli vault par basename (cible non résolue relativement)", async () => {
    ...
    const idx = fakeIndex({ "fiche-1": "/vault/fiche-1.md" });
    const forest = await build("/vault/notes/index.md", { readText: fs, getIndex: idx });
    expect(forest.displayPath).toBe("/vault/notes/index.md");
...
describe("buildTocForest — mémoïsation par hash structural (phases 6/6bis)", () => {
  ...
  test("frappe dans le CORPS (hash structural identique) → hit memo", async () => {
    // Le buffer live (referenceSource) fait foi — on simule une frappe dans
    // un paragraphe : le fichier de référence lui-même n'est jamais relu.
  ...
  test("changement de fichier de référence (clé) → rebuild", async () => {
    ...
    const first = await buildTocForest({ ...opts, referencePath: "/vault/index.md" }, memo);
    const rebuilt = await buildTocForest({ ...opts, referencePath: "/vault/autre.md" }, memo);
    expect(rebuilt).not.toBe(first);
    expect(rebuilt.displayPath).toBe("/vault/autre.md");
    expect(collectHeadings(rebuilt.root!)).toEqual(["Autre"]);
  });
